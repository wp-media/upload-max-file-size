...
class UMFS_Plugin_Card_Helper {
	...
	public function get_status_text( $status = null ){
		...
		$st = array(
			'activated' => __( 'activated', 'upload-max-file-size' ),
			'installed' =>  __( 'installed', 'upload-max-file-size' ),
			'not_installed' => __( 'not installed', 'upload-max-file-size' ),
		);
	...
	public function get_button_text( $status = null ){
		$s = ( is_string( $status ) && !empty( $status ) )?$status:$this->get_status();
		$bt = array(
			'activated' => __( 'Already activated', 'upload-max-file-size' ),
			'installed' =>  __( 'Activate plugin', 'upload-max-file-size' ),
			'not_installed' => __( 'Install plugin', 'upload-max-file-size' ),
		);
	...
	public function install_callback(){
		...
		if( $this->args['force_activation'] ){
			...
			$notices->append( 'success', __( 'This plugin has been successfully installed and activated.', 'upload-max-file-size' ) );
		}else{
			$notices->append( 'success', __( 'This plugin has been successfully installed.', 'upload-max-file-size' ) );
		}
	...
	public function activate_callback(){
		...
		$notices->append( 'success', __( 'This plugin has been successfully activated.', 'upload-max-file-size' ) );
		wp_safe_redirect( wp_get_referer() );
	}
	...
	protected function default_helper(){
		...
		$helper = $this;
		$file_paths = array(
			UMFS_PLUGIN_PATH.'views/plugin-cards/'.$this->plugin_slug.'.php',
			UMFS_PLUGIN_PATH.'views/plugin-cards/default.php',
		);
